 ...
 * @arg {String} [delimiter] Option to provide a custom delimiter. Defaults to `"."`
 * @arg {String} [devComment] Optional comment to set for this node.
 * @return {Struct.TagNode} */
function TagNode(tagName, fullTag, delimiter = ".", devComment = undefined) constructor {
	...
	/// The depth of this node in its hierarchy. A depth of 0 indicates a root node
	self.depth = 0;
	
	//Figure out if we should have a parent or if we're a root node.
	var parentName = string_replace(fullTag, $"{delimiter}{tagName}", "");
	if (!is_undefined(parentName) && (parentName != "") && (parentName != tagName)) {
		//Set our parent and add ourselves as its child
		parentNode = TagManager_Get().FindOrCreateNode(parentName, delimiter);
	...
	/** Returns the depth of this node within its hierarchy. A depth of 0 indicates that this node is the root.
	 * @return {Real} */
