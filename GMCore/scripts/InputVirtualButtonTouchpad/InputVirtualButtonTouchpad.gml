//feather ignore all
/** InputVirtualButtonTouchpad: A type of `InputVirtualButton` that represents a virtual touchpad.
 * @ignore
 ...
 * @return {Struct.InputVirtualButtonTouchpad} */
function InputVirtualButtonTouchpad(_click, _up, _down, _left, _right, _position, _size) : InputVirtualButton(_click, _position, _size) constructor {
	...
	#region Internal
		...
		static ButtonFollowCapture = function(_valueRawArray, _valueClampArray, _direction) {
			...
			if (!is_undefined(_left)) {
				_valueRawArray[_left] = max(0, -_direction.x);
				_valueClampArray[_left] = max(0, -_direction.x);
			}
			
			if (!is_undefined(_up)) {
				_valueRawArray[_up] = max(0, -_direction.y);
				_valueClampArray[_up] = max(0, -_direction.y);				
			}
			
			if (!is_undefined(_right)) {
				_valueRawArray[_right] = max(0, _direction.x);
				_valueClampArray[_right] = max(0, _direction.x);
			}
			
			if (!is_undefined(_down)) {
				_valueRawArray[_down] = max(0, _direction.y);
				_valueClampArray[_down] = max(0, _direction.y);			
			}						
		}			
