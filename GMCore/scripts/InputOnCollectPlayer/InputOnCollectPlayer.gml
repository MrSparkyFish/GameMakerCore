...
/** InputOnCollectPlayer: Represents an input player collection event
 * ***
 * Inherits: `MulticastAction`
 * @return {Struct.InputOnCollectPlayer} */
function InputOnCollectPlayer() : MulticastAction() constructor {
	 ...
	 * * `{Struct.InputPlayer}` *_player* The player that had their data collected
	 ...
	 * @arg {Struct.InputPlayer} _player The player that had their data collected
	 * @return {Undefined} */
	static Broadcast = function(_player) {
		///@ignore
		static parentBroadcast = MulticastAction.Broadcast;
		parentBroadcast([_player]);
	}				
}