 ...
 * @return {Struct.DynamicStructure} */
function DynamicStructure(dynamicResource, resourceOwner, destructor, args = undefined) constructor {
	...
	if (DYNAMIC_GARBAGE_SAFETY_CHECKS) {
		...
		if (resourceOwner == dynamicResource) {
			ThrowInvalidData("DynamicStructure", $"Supplied dynamic data cannot be the same as the supplied data owner.");
		}
